...
//      p
        ...
        surfaceVectorField gradPf ("gradPf", test.faceScalarGrad(p));
...
//      tau
//  surfaceScalarField tauQGDf("tauQGD", alphaQGD * hQGD / (cf ));
        surfaceScalarField tauQGDf("tauQGD", alphaField * hQGD / (cf ));
// Fluxes for first equation
        phi = phivf*rhof;

        surfaceVectorField rhoW1("rhoW1", tauQGDf * test.faceTensorDiv(rho * (U * U)));
        surfaceScalarField phiRhoW1("phiRhoW1", rhoW1 & mesh.Sf());

        surfaceVectorField rhoW2 = tauQGDf * gradPf;
        surfaceScalarField phiRhoW2("phiRhoW2", rhoW2 & mesh.Sf());
...
// term for q
        ...
        (
          "qf",
            - tauQGDf * ( ( (fvc::interpolate(rho*(U*U))) & test.faceScalarGrad(e)) + (pf * rhof * Uf * (Uf & test.faceScalarGrad(1/rho))))
        );
